...
public class WebCrawler extends JFrame
{
    ...
    static void executeTask(String url, int depth)
    {
        ...
        if (!("".equals(fetchedHTML) || "".equals(title))) LINKS.put(url, title);
    ...
    private void startFetching()
    {
        if (!fetching)
        {
            ...
            int maxExecutionTime = timeCheckBox.isSelected() ? Integer.parseInt(timeTextField.getText()) : 0;
            ...
            do
            {
                ...
                {
                    if (DEBUG) System.err.printf("InterruptedException! %s%n", e);
                }
    ...
    private void startExporting()
    {
        ...
        {
            for (var entry : LINKS.entrySet()) pw.printf("%s%n%s%n", entry.getKey(), entry.getValue());
    ...
    {
        if (fetchingStartTime == 0L) fetchingStartTime = System.currentTimeMillis();
        ...
        if (fetchingIsOver) fetchingStartTime = 0L;
    ...
    private static String fetchTextHtml(String url)
    {
        try
        {
            ...
            if (conn.getContentType() != null && !conn.getContentType().startsWith("text/html")) return "";
        ...
        {
            if (DEBUG) System.err.printf("Malformed URL (%s)!%n", url);
        }
        catch (IllegalArgumentException e)
        {
            if (DEBUG) System.err.printf("IllegalArgumentException! %s%n", url);
        }
        catch (IOException e)
        {
            if (DEBUG) System.err.printf("IOException! %s%n", e);
    ...
    {
        if (urlFragment == null || baseUrl == null) return "";
        
        if ("".equals(urlFragment) || "#".equals(urlFragment)) return baseUrl;
        
        var matcher = FULL_URL_PATTERN.matcher(urlFragment);
        if (matcher.matches()) return urlFragment;
        
        if (urlFragment.startsWith("www.")) return "http://" + urlFragment;
